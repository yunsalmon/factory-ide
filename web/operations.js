...
function renderOperations() {
  ...
  $('#ops-scope').onchange=e=>{S.operationScope=e.target.value;renderOperations();renderGraph();};
...
function operationCause(cause) {
  if (typeof cause === 'string' && cause.startsWith('buffer_full:')) return tr('graph_buffer_full',[cause.slice('buffer_full:'.length)]);
  if (operationStates.includes(cause)) return tr('ops_state_'+cause);
