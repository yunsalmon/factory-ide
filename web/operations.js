...
function operationView(result, cursor, final) {
  ...
  const current=copy(result?.initial_state?.machine_operations||Object.fromEntries((model?.machines||[]).map(m=>[m.id,{state:'idle',lot:null,since:0,cause:'initialized'}])));
  const resources=copy(result?.initial_state?.resources||{}), intervals=[], totals={};
...
function inspectOperations(id) {
  ...
  bindOperationsForm(async f=>{
    const matrix={};for(const row of $$('.ops-setup-row')){const values=[...row.querySelectorAll('input')].map(el=>el.value);if(!values[0]&&!values[1])continue;if(!values[0]||!values[1])throw new Error(tr('ops_invalid',['setup_matrix']));if(Object.hasOwn(matrix[values[0]]||{},values[1]))throw new Error(tr('ops_invalid',['setup_matrix']));(matrix[values[0]]||={})[values[1]]=Number(values[2]);}
    ...
    await mutate(model=>{
      ...
      target.resource_requirements={};for(const phase of ['setup','processing']){target.resource_requirements[phase]={};for(const r of model.resources||[]){const value=Number(f.get(`${phase}_resource_${r.id}`));if(value)target.resource_requirements[phase][r.id]=value;}}
...
function inspectSharedResources() {
  ...
  openInspector(tr('ops_resources'),`<form id="ops-form"><p>${opText('ops_atomic')}</p><div id="ops-resource-rows">${resources.map(resourceEditRow).join('')}</div><button type="button" id="ops-add-resource">${opText('ops_add')}</button><h3>${opText('ops_families')}</h3>${[...new Set(S.model.source.products)].map((product,i)=>`<label class="field">${esc(product)}<input class="ops-product-family" data-product="${esc(product)}" name="product_family_${i}" value="${esc(S.model.product_families?.[product]||product)}" required></label>`).join('')}<h3>${opText('ops_route_resources')}</h3>${S.model.routes.map((route,i)=>`<fieldset><legend>${esc(route.id)} (${esc(route.from)} → ${esc(route.to)})</legend>${resources.map(r=>opInput('ops_units',`route_${i}_${r.id}`,route.resources?.[r.id]||0,'number',`min="0" max="${r.capacity}" step="1" data-op-route="${esc(route.id)}" data-resource="${esc(r.id)}" required`)+`<small>${esc(r.id)}</small>`).join('')}</fieldset>`).join('')}${opsActions()}</form>`);
  ...
  bindOperationsForm(async()=>{
    ...
    const families=Object.fromEntries($$('.ops-product-family').map(el=>[el.dataset.product,el.value.trim()]));
    const requirements={};$$('#ops-form input[data-op-route]').forEach(el=>{if(Number(el.value))(requirements[el.dataset.opRoute]||={})[el.dataset.resource]=Number(el.value);});
    await mutate(model=>{model.resources=definitions;model.product_families={...(model.product_families||{}),...families};for(const route of model.routes)route.resources=requirements[route.id]||{};});inspectSharedResources();
  });
}
