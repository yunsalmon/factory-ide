...
function runDataWorker(preview=false){
 ...
 worker.onerror=()=>{if(id!==DATA.serial)return;worker.terminate();DATA.worker=null;DATA.busy=false;DATA.message={code:'data_worker_error'};if(S.tab==='data')renderDataPanel();};
...
async function readDataFile(file){
 ...
 try{const text=await file.text();if(id!==DATA.serial)return;DATA.text=text;DATA.busy=false;runDataWorker(DATA.format==='csv');}catch{if(id!==DATA.serial)return;DATA.busy=false;DATA.message={code:'data_read_error'};renderDataPanel();}
}
async function confirmDataImport(){
 const pending=DATA.pending;if(!pending||DATA.busy||PUBLIC_DEMO)return;
 ...
 try{
  ...
  else{await mutate(model=>{for(const key of Object.keys(model))delete model[key];for(const [key,value] of Object.entries(copy(pending.candidate)))Object.defineProperty(model,key,{value,writable:true,enumerable:true,configurable:true});});DATA.message={code:'data_model_applied'};}
  DATA.pending=null;
 }catch(error){DATA.diagnostics=[{table:'model',row:1,field:'model',code:error.detail?.code||'data_apply_error',args:error.detail?.args||[]}];DATA.message={code:'data_invalid'};}
 finally{DATA.busy=false;selectTab('data');}
}
...
function renderDataPanel(){
 ...
 $('#trace-content').innerHTML=`<section class="data-panel" aria-label="${tr('data_title')}"><p>${tr('data_private')}</p><div class="data-controls">
  ${PUBLIC_DEMO?'':`<label>${tr('data_file')}<input id="data-file" type="file" accept=".csv,.json,text/csv,application/json" ${DATA.busy?'disabled':''}></label><label>${tr('data_table')}<select id="data-table" ${DATA.busy?'disabled':''}>${Object.keys(DATA_FIELDS).map(table=>`<option value="${table}" ${table===DATA.table?'selected':''}>${tr('data_table_'+table)}</option>`).join('')}</select></label>`}
