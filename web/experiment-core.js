...
async function experimentDefinition(name,notes,source,model,seeds,concurrency){
 ...
 return {name:name.trim(),notes,source,model:JSON.parse(JSON.stringify(model)),seeds:[...seeds],concurrency,source_hash:await scenarioHash(source,true),model_hash:await scenarioHash(model)};
}
class ExperimentRunner{
 constructor({factory=()=>new BrowserPythonRuntime(),onUpdate=()=>{}}={}){this.factory=factory;this.onUpdate=onUpdate;this.workers=new Set();this.active=false;this.cancelled=false;}
 cancel(){if(!this.active)return;this.cancelled=true;for(const worker of this.workers)worker.stop();}
 async run(definition){
  if(this.active)experimentFail('ex_busy');definition=JSON.parse(JSON.stringify(definition));experimentSeeds(definition.seeds.join(','));if(![1,2].includes(definition.concurrency))experimentFail('ex_invalid');this.active=true;this.cancelled=false;
  ...
  const lane=async()=>{const worker=this.factory();this.workers.add(worker);try{while(!this.cancelled){const index=next++;if(index>=exp.runs.length)break;const row=exp.runs[index];row.status='running';this.onUpdate(exp);
   try{const payload=await worker.runSeed(definition.source,row.seed);if(this.cancelled)break;
    const result=payload.result,contract=scenarioContract(result),expected=JSON.parse(JSON.stringify(definition.model));expected.seed=row.seed;
    const input_model_hash=await scenarioHash(payload.input_model??result.model);
    if(await scenarioHash(expected)!==input_model_hash)experimentFail('ex_model');
    ...
    const runtime=JSON.parse(JSON.stringify(worker.runtime)),trace_hash=await scenarioHash(result),source_hash=await scenarioHash(payload.source,true),model_hash=await scenarioHash(result.model);
   ...
   }catch(error){if(!this.cancelled)Object.assign(row,{status:'failed',runtime:worker.runtime?JSON.parse(JSON.stringify(worker.runtime)):null,error:String(error.error||error.message||error).slice(0,2000)});}
...
async function experimentExport(experiments){const doc={format:'factory-experiments',version:1,experiments:JSON.parse(JSON.stringify(experiments))};const artifact={...doc,content_hash:await scenarioHash(doc)};if(new TextEncoder().encode(JSON.stringify(artifact)).length>EXP_LIMITS.artifact)experimentFail('ex_limit');return artifact;}
async function experimentRestore(doc){
 ...
 if(doc.content_hash!==await scenarioHash({format:doc.format,version:doc.version,experiments:doc.experiments}))experimentFail('ex_integrity');
 for(const exp of doc.experiments){if(exp.version!==1)experimentFail('ex_invalid');const d=exp.definition,checked=await experimentDefinition(d.name,d.notes,d.source,d.model,d.seeds,d.concurrency);if(checked.source_hash!==d.source_hash||checked.model_hash!==d.model_hash||!Array.isArray(exp.runs)||exp.runs.length!==d.seeds.length)experimentFail('ex_integrity');
  ...
  const successful=exp.runs.filter(r=>r.status==='success');if(successful.length&&!exp.representative)experimentFail('ex_integrity');for(const row of successful){const expected=JSON.parse(JSON.stringify(d.model));expected.seed=row.seed;if(await scenarioHash(expected)!==row.input_model_hash||!row.runtime||typeof row.runtime!=='object'||![row.source_hash,row.trace_hash].every(h=>typeof h==='string'&&/^[a-f0-9]{64}$/.test(h)))experimentFail('ex_integrity');}
  if(exp.representative){const {index,result,source}=exp.representative,row=exp.runs[index];if(!row||row.status!=='success'||await scenarioHash(result)!==row.trace_hash||await scenarioHash(source,true)!==row.source_hash||await scenarioHash(result.model)!==row.model_hash||scenarioCanonical(scenarioContract(result))!==scenarioCanonical(exp.contract)||scenarioCanonical(scenarioMetrics(result))!==scenarioCanonical(row.metrics))experimentFail('ex_integrity');}
  experimentAggregate(exp);
 }
 return JSON.parse(JSON.stringify(doc.experiments));
}
