...
- `due_date`: optional RFC3339 datetime with full seconds and explicit timezone (`Z` or `±HH:MM`), optionally 1–3 fractional second digits. Requires `MODEL.time_origin` in the same format. The normalized due minute is `(due_date - time_origin) / 60 seconds`. If both due fields exist, they must agree within 1e-7 minutes. Original date/priority/customer/reference values remain unchanged in the model, export and lot `order` header; UTC conversion is only derived metadata. Dates without a timezone are rejected. Offset hours must be 00–23 and minutes 00–59; values such as `+00:60` are rejected before Python can normalize them, so the pre-run JavaScript projection and trace normalization agree.
...
Trace `schema_version` stays 2 with additive `order_schema_version: 1` and `order_plan`. In order mode, machine transitions before the first release (including an empty order plan) are retained as events with `lot: null`, `affected_lot_id: null` and an empty `state_changes.lots`. Consumers must read the machine changes without inventing a lot. The event inspector and allocation/replay views accept this envelope. At time zero, when an actual release is scheduled at zero, zero-duration machine changes are coalesced into that first lot snapshot to preserve its established event ordering. Generated legacy supply keeps its previous envelope/order behavior. The #10 operational inventory adapter accepts the release placement and preserves its identity; buffer queues exclude it. WIP consumers should show this as a distinct external release queue and must not count it as INPUT buffer occupancy.
...
The planner owns `PLANNER` filters, final mode and selection without touching WIP/result filters. `orderProjection` is pure and exposes order rows, planned/released lot drilldown, location quantities, prefix-bounded operations/history/allocation indexes and filtered/unfiltered totals. It accepts optional `options.inventory.rows` from a WIP projection; the UI supplies `inventoryProjection` when installed. For legacy snapshots without explicit placement, inferred WIP locations remain usable. For snapshots with physical placement, a shared adapter must advertise `location_contract: "factory-placement-v1"`, and its row must carry the same `placement: {kind,id}` and raw `state` as the snapshot before its `location`, `node`, and `operation` are used. Rows always retain authoritative placement/state. This versioned contract distinguishes real buffer storage from external release backlog.

The actual #11 adapter at `b0d559b696d0d9123761d1d95dacbd5e011c7808` has no such contract and collapses both kinds to INPUT. Therefore its incompatible rows cannot override #12's authoritative operational locations. The fixed #11 source is included unchanged as a test fixture, and both pure-function and real browser tests load it alongside the planner. `orderLotLocation` handles #10 buffer placements (`buffer.id` → `buffer.at`), machine/transport and external release placements whenever a compatible WIP row is unavailable. This keeps later #11 integration additive rather than requiring its branch now.
