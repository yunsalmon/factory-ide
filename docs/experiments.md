...
These experiment Workers use the `experiment` role from the versioned
[browser Worker resource contract](worker-resources.md). Their settled count is
zero. The one reusable interactive Python Worker allowed while the public page
is open is tracked separately and does not belong to the experiment pool.
