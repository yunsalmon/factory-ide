...
**전체 할당 결과** 탭은 성공한 할당 ID마다 한 행을 표시합니다. 같은 로트의 후속 공정 방문과 머신 재방문은 서로 다른 행입니다. 로트/제품, 출발 및 목적 공정·라인·머신, 경로, 방식, 실제 할당·이동 시작·도착·처리 시작·종료 시각과 해당 방문의 상태를 확인할 수 있습니다. INPUT/OUTPUT의 공정·라인과 발생하지 않은 시각은 비어 있습니다. 기본 모델은 머신 도착과 처리 시작이 동시이므로 `start`가 도착 시각이기도 합니다. [운영 모델 확장](operational-model.md)의 setup/down/offshift를 사용하면 실제 도착은 `transport_arrive`, 실제 처리 구간은 `machine_operations`에 기록됩니다. 기존 표의 도착/처리 구간 표시는 이 확장을 아직 반영하지 않습니다. OUTPUT 도착은 `complete`에서 가져옵니다. `duration`이나 경로 지연으로 미래 완료/도착을 예측하지 않습니다.
