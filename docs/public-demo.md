...
The Data calibration tab supports browser-local CSV/JSON file selection. Select a CSV table and map columns, or select a versioned JSON model/observations document, then explicitly run validation. Correct the reported rows/fields in the original file and select it again. Review the candidate summary and use the separate confirm button to apply it. Cancel, validation failure, and an outdated preview preserve the current model; edits during asynchronous confirmation invalidate that confirmation. A confirmed model can be simulated immediately. Raw files/observations stay in page memory and are not uploaded; only explicitly confirmed model source uses the existing browser-local persistence. Export is an explicit download.
